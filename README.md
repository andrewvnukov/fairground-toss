...
`__GAME_TITLE__`, `__SAVE_KEY__`, `__LEADERBOARD__` — всегда. `__ACTION_LABEL__`/`__UPGRADE_LABEL__` + `__ACTION_RU__`/`__ACTION_EN__`/`__UPGRADE_RU__`/`__UPGRADE_EN__` — опционально: только если под кнопку есть реальное не-тап действие, см. RUNBOOK.md → «Кнопки: только необходимые». Если действие тап-based (кормить/бросать/собирать/гладить), кнопку и её плейсхолдеры удаляют, а не заполняют.

Также добавь переключатель языка `#langBtn` (обязателен в каждой игре) — см. RUNBOOK.md → «Переключатель языка RU/EN».
...
Если браузеры Playwright уже лежат в системе, путь к бинарю можно задать
переменной `PW_CHROMIUM`.

## Прогон баланса

Баланс проверяется не чтением формул, а прогоном: `test/balance.mjs` гоняет
симуляцию без интерфейса и печатает **время до вех в минутах активной игры**.

```
node test/balance.mjs            # «оптимальный» бот
node test/balance.mjs greedy     # «жадный» — берёт самое дешёвое
node test/balance.mjs stubborn   # «упрямый» — копит только на одну ветку
```

Ориентир: весь маршрут — 3–7 дней при трёх заходах по 13 минут.
